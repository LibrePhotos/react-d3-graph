...
function _renderLinks(nodes, links, linksMatrix, config, linkCallbacks, highlightedNode, highlightedLink, transform) {
  ...
  return outLinks.map(link => {
    ...
    const sourceId = (0, _graph3.getId)(source);
    const targetId = (0, _graph3.getId)(target);
    const key = `${sourceId}${_graph.default.COORDS_SEPARATOR}${targetId}`;
    const props = (0, _graph2.buildLinkProps)(
      _objectSpread(
        _objectSpread({}, link),
        {},
        {
...
function _renderNodes(nodes, nodeCallbacks, config, highlightedNode, highlightedLink, transform, linksMatrix) {
  ...
  if (config.collapsible) {
    outNodes = outNodes.filter(nodeId => (0, _collapse.isNodeVisible)(nodeId, nodes, linksMatrix));
  }

  return outNodes.map(nodeId => {
    const props = (0, _graph2.buildNodeProps)(
      _objectSpread(
...
function _renderDefs() {
  ...
  return config => {
    ...
    if (!markerCache[key]) {
      const { small, medium, large } = (0, _marker2.getMarkerSize)(config);
      const markerProps = {
        markerWidth: config.link.markerWidth,
        markerHeight: config.link.markerHeight,
      };
      markerCache[key] = /*#__PURE__*/ _react.default.createElement(
        "defs",
        null,
